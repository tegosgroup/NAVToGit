function Open-SelectiveExportMenu {
    param(
        [switch]$dark,
        [switch]$nav6
    )
    ...
    $SelectiveExport = New-Object system.Windows.Forms.Form
    $SelectiveExport.ClientSize = '340,480'
    $SelectiveExport.text = "Selective Export"
    $SelectiveExport.MaximizeBox = $false
    $SelectiveExport.FormBorderStyle = 'Fixed3D'
    $SelectiveExport.TopMost = $false
    $SelectiveExport.StartPosition = 1
    ...
    $NoteGroupBox.height = 90
    $NoteGroupBox.text = "Note"
    $NoteGroupBox.location = New-Object System.Drawing.Point(16, 366)

    $NoteLabel = New-Object system.Windows.Forms.Label
    $NoteLabel.Text = "Please enter the filter just as you would in Dynamics NAV. Empty filters will be ignored and no object of this type will be exported." + [System.Environment]::NewLine + "Example: id=1337..1360;modified=yes"
    ...
    if ($nav6) {
        ...
        $SelectiveExport.ClientSize = '340,497'
...
function Invoke-OKButtonClick {
    ...
    [String]$argumentlist = '-noExit -command "$ExportFromNAVToGit = Join-Path -Path (Split-Path (Split-Path -Parent (""' + $PSScriptRoot + '""")) -Parent) -ChildPath "public\Export-FromNAVToGit.ps1";. $ExportFromNAVToGit; Export-FromNAVToGit -customfilter ""' + $CustomFilter + '"""'
    Start-Process powershell.exe -ArgumentList $argumentlist
    $SelectiveExport.Close()
}
...
function Set-DarkMode {
    $SelectiveExport.BackColor = "#383838"
